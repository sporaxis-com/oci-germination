...
func pgckSecretsConf(seed, workerPass string, oidc oidcConfig) string {
	...
	// Canonical lowercase kernel name. pgCK 0.4.77's dispatcher refuses its own
	// compiled-in GUC default 'pgCK' as non-canonical case ("use 'pgck'"), so
	// every in-band verb dies before attribution unless the bundle pins the
	// canonical form. Case is a live trap: a segment in the wrong case matches
	// nothing and is indistinguishable from never having been configured.
	fmt.Fprintf(b, "pgck.kernels = 'pgck'\n")
...
func anonPgckConf() string {
	// pgck.kernels: same canonical-case pin as the realm fragment — see
	// pgckSecretsConf.
	return fmt.Sprintf(`# GENERATED by ociger-ck-identity at boot — DO NOT EDIT.
pgck.kernels = 'pgck'
