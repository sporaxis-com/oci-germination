...
### v0.7.30 — UNRELEASED (in prep on `v0.7.30-prep`) — delete the Go relay; pgCK 0.4.24 owns inbound identity
- **Tried:** adopt **pgCK 0.4.24-pg18-nats** (base `pg18-pgrdf-pgck-nats-micro` v0.2.0 → **v0.2.1**). pgCK 0.4.24 lands server-derived dispatch identity end-to-end (F1-inbound, **hop 4**): the `-nats` build answers the NATS auth-callout (`$SYS.REQ.USER.AUTH`), binds the broker-enforced verified `<sub>` to `ckp.requester` **before** `ckp.dispatch`, and owns the in-extension inbound dispatch **and** the `ckp.outbox` drain. Consequently the Go **`ociger-pgck-relay` is DELETED** — build stage, binary COPY, s6 service, `cmd/ociger-pgck-relay`, and `OCIGER_DISABLE_OUTBOX_DRAIN` all removed — because running it alongside 0.4.24 would **double-dispatch** every inbound action. Broker-enforced **id-scoped subject grammar**: a verified connection may publish only on `input.kernel.pgCK.id.<sub>.action.>` (legacy `…action.<verb>` routes anonymous). **Auth-callout wired** via a new boot-time provisioner **`ociger-ck-identity`** (s6 oneshot, before nats + postgres): it resolves the callout ACCOUNT nkey (`OCIGER_NATS_ACCOUNT_SEED` if operator-supplied → stable; else minted per-boot) and writes both `/etc/nats/nats-server.conf` (auth_callout `issuer` = account pubkey; `pgck_worker` the sole bypass user) and `/run/ck-identity/pgck.conf` (`pgck.nats_account_seed` + worker-cred `pgck.nats_url`) from **one** account, so the responder's signing seed always matches the broker's issuer. postgres pulls the fragment via `include_if_exists` every boot; the seed is **never baked into the image**. This replaces the interim `*.sealed` forge-deny (the callout anon tier is subscribe-only). Client id-scoped publish subject tracked in **ConceptKernel/CK.Lib.Js#11**.
- **Tested:** `go test ./...` green including the new **`ociger-ck-identity`** unit suite (mints round-tripping account nkeys, honors an operator seed, rejects non-account/garbage seeds, URL-encodes worker creds, renders callout config with no `no_auth_user`). Drift gate green (pgck=0.4.24). **Not container-built (build freeze).** Build-time re-validation gates when the freeze lifts: (a) §B4's reply envelope (`{ok,proof_digest,id}`) is now emitted by pgCK's in-extension dispatch — confirm shape against 0.4.24; (b) s6 oneshot ordering + `include_if_exists` seed hand-off end-to-end; (c) the verified/anon `by:` "after" assertion.
- **Verdict:** **UNRELEASED / IN PREP** on `v0.7.30-prep`. Seed-delivery design **RESOLVED** (boot-gen + operator override). Remaining gates: (1) a **verified-realm bearer** to prove the `by:`-present / anon-absent "after" locally; (2) the **build freeze lifted** for the base-first CI cut. Closes **og#22**; integrates **og#19**; companion **CK.Lib.Js#11**.
