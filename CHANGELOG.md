...
- **Tested:** a new **cross-arch parity gate** runs *before* attestation and asserts every published arch shares (1) the same `image.revision` equal to the building commit, (2) the same runtime glibc, and — the check that actually catches the defect above — (3) the **same maximum GLIBC floor across all shipped extension `.so`**. A revision+libc check alone passes v0.7.30 and would have missed this entirely; floor parity is the property that matters. Component pins unchanged (pgRDF 0.6.20, pgCK 0.4.24-nats, cklib 1.5.5); drift gate green; base v0.2.2 rebuilt locally and glibc-verified.
- **Verdict:** **NOT CUTTABLE YET — intentionally.** The floor-parity gate **fails against pgRDF 0.6.20** (2.34 vs 2.39), which is the correct fail-closed outcome: v0.7.31 waits on a consistently-built **pgRDF 0.6.21**. Shipping the gate and exempting ourselves from it would be the same overclaiming this release exists to end. **Still open (toward sporaxis I9 / sporaxis-com/sporaxis#31):** upstream is pinned by **mutable tag** and the drift gate compares **version strings**, not digests — the structural fix is digests as composition facts, with `sporaxis check` refusing a tag-pinned composition.
